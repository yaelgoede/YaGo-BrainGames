...
export default function SpeedComparePage() {
  ...
  const handlePick = (side: "left" | "right") => {
    ...
    if (side === correct) {
      ...
      flashTimeout.current = setTimeout(() => {
        setFlash(null);
        setPair(generatePair(Math.floor(newScore / 5) + 1));
      }, 300);
    } else {
      setPhase("over");
    }
  ...
  return (
    <GameShell
      gameId="speed-compare"
      title="Speed Compare"
      score={score}
      onRestart={start}
      showTimer={phase === "playing"}
      timeLeft={timeLeft}
      instructions="Two numbers appear — tap the larger one! Get it wrong and the game is over. Numbers get trickier as your streak grows."
    >
      {phase === "idle" && (
          ...
          <div className="flex gap-6">
            {(["left", "right"] as const).map((side) => (
              <button
                ...
                className={`flex h-36 w-36 items-center justify-center rounded-2xl border-2 text-4xl font-bold transition-all duration-150
                  ${flash === side
